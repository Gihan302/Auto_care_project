...
export default function ApplyForAdPage() {
  ...
  useEffect(() => {
    ...
    const fetchAdDetails = async () => {
      try {
        const response = await api.get(`/advertisement/${adId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.post('/applications/submit', { ...formData, adId });
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.adSummary}>
        <img src={adDetails.images[0]?.imageUrl || '/placeholder.jpg'} alt={adDetails.title} />
        <div>
